...
;; Custom additions implemented in initial implementation,
;; need to be ported to separate analyzer/evaluator impl.
;;
;; ((make-unbound? exp) (eval-make-unbound! exp env))
;; ((and? exp) (eval-and (cdr exp) env))
;; ((or? exp) (eval-or (cdr exp) env))

;; Saved for reference until implemented in analyzer
  ...
  (define (body-loop body)
    (if (null? body)
        ...
        (if (definition? (car body))
            (body-loop (cdr body))
            (cons (car body) (body-loop (cdr body))))))
  (define (defines-loop body)
    (if (null? body)
        '()
        (if (definition? (car body))
            (cons (list 'set!
                  ...
                  (defines-loop (cdr body)))
            (defines-loop (cdr body)))))
  (define (bindings-loop body)
    (if (null? body)
        '()
        (if (definition? (car body))
            (cons (list (definition-variable (car body)) '*unassigned*)
                  (bindings-loop (cdr body)))
            (bindings-loop (cdr body)))))
  (let ((clean-body (body-loop proc-body))
        (define-bodies (defines-loop proc-body))
        (bindings (bindings-loop proc-body)))
    (append (list 'let bindings) define-bodies clean-body)))
  ...
  (define (env-loop env)
    (define (scan bindings)
      (cond ((null? bindings)
            ...
            ((null? (car bindings))
             (scan (cdr bindings)))
            ((eq? '*unassigned* (cadar bindings))
             (error "The variable has not yet been assigned a value."))
